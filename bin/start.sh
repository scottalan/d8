...
if [[ $DOCKER_ENV == 'local' ]]; then
  ...
  export COMPOSE_PROJECT_NAME=${2-'d8_'${DOCKER_ENV}}
fi
...
cmd "docker-compose -f docker-compose.yml up -d www"

# Correct any issues in the web container.
cmd "docker exec d8_${DOCKER_ENV}_www \"/var/www/bin/fix-perms.sh\""
...
URL=$(docker-compose -f build$COMPOSE_EXT.yml run --rm drush sa @d8 --format=list --fields=uri)
echo "$URL"
echo
